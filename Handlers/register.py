...
async def start(message: Message, state: FSMContext):
    ...
    elif DBfunc.IF('teacher','`TelegramID`',f'`TelegramID` = {message.from_user.id}'):#Если пользователь авторизован как учитель
        data = DBfunc.SELECT('`FSc`','teacher',f'`TelegramID` = {message.from_user.id}')[0][0] #Полуаем данные из БД
        await message.answer(f'Привет {data.split()[1]}. Чем могу быть полезен?',reply_markup=mainKeyboard())
...
async def ceckid(message: Message, state: FSMContext):
    try:
        if DBfunc.IF('student','*',f'`user key` = "{message.text}" AND TelegramID != {message.from_user.id}'): #Проверяем через БД условие если key указан верно и TelegramID != UserTelegramID
            FSc = DBfunc.SELECT('`id`,`FSc`','student',f'`user key` = "{message.text}"')[0] #Получаем ФИО и id пользователя по ввведенному ключу
